...
def check_horizontale(grille, x, y):
    ...
    if grille.is_far_from_left(x) and all(
        symbole == grille.grid[y][x - i - 1] for i in range(2)
    ):
...
def check_verticale(grille, x, y):
    ...
    if (
        grille.is_quite_far_from_bottom(y)
        and not (grille.is_at_top(y))
        and all(symbole == grille.grid[y + i + 1][x] for i in range(2))
    ):
...
def check_oblique_montante(grille, x, y):
    ...
    if (
        grille.is_far_from_top(y)
        and grille.is_far_from_right(x)
        and all(symbole == grille.grid[y - i - 1][x + i + 1] for i in range(2))
    ):
    ...
    if grille.is_far_from_bottom(y) and grille.is_far_from_left(x):
        ...
        if all(
            symbole == grille.grid[y + i + 1][x - i - 1] for i in range(2)
        ) and grille.is_very_far_from_bottom(y):
