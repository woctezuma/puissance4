...
class MC(AI):
    ...
    def __init__(
        ...
        symbole='O',
        num_tirages_MC=None,
        deterministic_sampling_of_actions_at_root=None,
    ):
        ...
        if deterministic_sampling_of_actions_at_root is not None:
            self.deterministic_sampling_of_actions_at_root = (
                deterministic_sampling_of_actions_at_root
            )
        else:
            self.deterministic_sampling_of_actions_at_root = (
                get_default_action_sampling_strategy()
    ...
    def print(self):
        super().print()
        print(
            "[Monte Carlo] number of samples for grid valuation = {}".format(
                self.num_tirages_MC,
            ),
        )
        self.print_strategy()
    ...
    def print_strategy(self):
        print(
            "[Monte Carlo] deterministic strategy for action sampling at the root = {}".format(
                self.deterministic_sampling_of_actions_at_root,
            ),
        )
        return
    ...
    def play_witout_bias(self, grille):
        ...
        if len(allowed_steps) == 1:
            ...
            if self.deterministic_sampling_of_actions_at_root:
                meilleure_action = self.uniformly_sample_actions_at_root(
                    grille,
                    allowed_steps,
                )
            else:
    ...
    def randomly_sample_actions_at_root(self, grille, num_end_game_simulations=None):
        ...
        _, meilleure_action = self.simuler_monte_carlo(
            grille,
            current_player,
    ...
    def uniformly_sample_actions_at_root(
        self,
        grille,
        allowed_steps=None,
        ...
        for action in allowed_steps:
            ...
            R, _ = self.simuler_monte_carlo(
                grille_simulee,
                self.get_opponent_symbol(),
                num_end_game_simulations,
            )
    ...
    def simuler_monte_carlo(
        ...
        victory_stats_for_first_actions = self.core_process_monte_carlo(
            grille,
            current_player,
            num_end_game_simulations,
        )

        best_grid_valuation, best_first_action = self.post_process_monte_carlo(
    ...
    def core_process_monte_carlo(
        ...
        for _ in range(num_end_game_simulations):
            ...
            try:
                ...
                victory_stats_for_first_actions[first_action] = {
                    'O': 0,
                    'X': 0,
                    'draw': 0,
                }
    ...
    def post_process_monte_carlo(self, victory_stats_for_first_actions, current_player):
        grid_valuations_for_first_actions = self.find_grid_valuations_for_first_actions(
            victory_stats_for_first_actions,
            current_player,
        )

        best_first_action = self.find_best_first_action(
            grid_valuations_for_first_actions,
        )

        best_grid_valuation = self.find_best_grid_valuation(
            grid_valuations_for_first_actions,
            best_first_action,
        )
    ...
    def find_grid_valuations_for_first_actions(
        self,
        victory_stats_for_first_actions,
        player_symbol,
    ):
        ...
        for action, num_victoires in victory_stats_for_first_actions.items():
            try:
                action_score = (
                    ...
                    num_victoires[player_symbol]
                    + num_victoires[opponent_symbol]
                    + num_victoires['draw']
                )
    ...
    def find_best_first_action(cls, grid_valuations_for_first_actions):
        ...
        best_first_action = max(
            grid_valuations_for_first_actions.items(),
            key=operator.itemgetter(1),
        )[0]
        return best_first_action

    def find_best_grid_valuation(
        self,
        grid_valuations_for_first_actions,
    ...
    def equalize_computing_resources(self, UCT_ai_instance):
        ...
        try:
            ...
            self.num_tirages_MC = (
                UCT_ai_instance.num_tirages_MC
                * UCT_ai_instance.num_descentes_dans_arbre
            )
